...
const DealerDashboard = () => {
  ...
  },
  {
    ...
    customer: {
      ...
      avatarAlt: "Professional headshot of man with beard wearing dark suit and tie"
    },
    vehicle: {
      year: "2023",
      make: "Chevrolet",
  ...
  },
  {
    ...
    customer: {
      ...
      avatarAlt: "Professional headshot of woman with blonde hair in gray blazer"
    },
    vehicle: {
      year: "2024",
      make: "BMW",
  ...
  useEffect(() => {
    ...
    // Calculate metrics
    const activeDeals = mockDeals?.filter((deal) => deal?.status !== 'completed')?.length;
    const completionRate = Math.round(mockDeals?.filter((deal) => deal?.status === 'completed')?.length / mockDeals?.length * 100);
    const urgentItems = mockDeals?.filter((deal) => deal?.status === 'urgent' || deal?.priority === 'high')?.length;
    const pendingDocuments = mockDeals?.reduce((sum, deal) => sum + deal?.pendingDocuments, 0);
  ...
  useEffect(() => {
    ...
    if (filters?.urgency !== 'all') {
      filtered = filtered?.filter((deal) => deal?.priority === filters?.urgency);
    }

    // Staff filter
    if (filters?.assignedStaff !== 'all') {
      filtered = filtered?.filter((deal) => deal?.assignedStaff === filters?.assignedStaff);
    }
  ...
  const handleDealClick = (deal) => {
    navigate('/conversation-interface', { state: { dealId: deal?.id, customer: deal?.customer } });
  };
  ...
  const handleDocumentRequest = (deal) => {
    navigate('/document-management', { state: { dealId: deal?.id, customer: deal?.customer } });
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-6 py-8">
        {/* Page Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-semibold text-foreground mb-2">Dealer Dashboard</h1>
